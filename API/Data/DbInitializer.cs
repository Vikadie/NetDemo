...
namespace API.Data
{
    public static class DbInitializer // static is used in order not to need to instantianate this class
    {
        public static async Task Initialize(StoreContext context, UserManager<User> userManager) // because of userManager the methods should be transferred from void -> async Task
        {
            ...
            foreach ( var product in products) // loop over the list above
            {
                context.Products.Add(product); // Entity framework tracks that we want to add them at some point in the future
            }

            context.SaveChanges(); // this is the point in the future, where all changes in teh DB will be stored in out DB
        }
    }
}